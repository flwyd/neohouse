...
import {AmericanSoundexRule, DoubleMetaphoneRule, EditDistanceRule, EyeRhymeRule, FccRule, HandleConflict, MinLengthRule, PhoneticAlphabetRule, SubstringRule} from '../utils/handle-rules';
...
export default Controller.extend({
  ...
  handleRules: computed('model', function() {
    const handles = this.get('model').toArray();
    const rules = {};
  ...
  actions: {
    ...
    checkCurrentName() {
      const name = this.get('currentName');
      const conflicts = [];
      const rules = Object.values(this.get('handleRules')).map((obj) => obj.rule);
      const id = nextCheckId++;
      rules.map((rule) => conflicts.push(...rule.check(name)));
      conflicts.sort(HandleConflict.comparator);
      this.get('checkedHandles').unshiftObject({id: id, name: name, conflicts: conflicts});
